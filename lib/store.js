...
const BACKGROUNDS = [
  'confetti', 'pastel-balloons', 'golden-elegance', 'tropical-fiesta', 'starry-night',
  'sunset-blush', 'ocean-breeze', 'emerald-garden', 'berry-sparkle', 'classic-ivory'
];
...
function makeStore(dataDir) {
  ...
  // source: 'admin' (host generated it to send to a specific family) or
  // 'public' (auto-minted for someone who came through the shared join
  // link) — the admin UI uses this to decide whether showing/copying this
  // invite's own link is useful, since a 'public' visitor already has
  // access via the link they used.
  function createInvite(eventId, familyLabel, source = 'admin') {
    const invite = {
      id: makeId(),
      eventId,
